...
import com.google.ai.edge.gallery.ui.common.chat.sendRobotCommand
...
fun ChatViewWrapper(
  ...
  ChatView(
    ...
    navigateUp = navigateUp,
    modifier = modifier,
    onRobotCommand = if (viewModel.task.type == TaskType.LLM_ASK_IMAGE) { robotIP, command ->
      scope.launch {
        val success = sendRobotCommand(robotIP, command)
        if (success) {
          Log.d("RobotControl", "Successfully sent command '$command' to robot at $robotIP")
        } else {
          Log.e("RobotControl", "Failed to send command '$command' to robot at $robotIP")
        }
      }
    } else null,
  )
