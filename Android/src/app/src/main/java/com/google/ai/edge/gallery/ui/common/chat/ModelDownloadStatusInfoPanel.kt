...
fun ModelDownloadStatusInfoPanel(
  ...
  // Debug logging
  LaunchedEffect(curStatus) {
    Log.d("ModelDownloadStatusInfoPanel", "Model '${model.name}' status: ${curStatus?.status}")
  }
  ...
  AnimatedVisibility(visible = shouldShowDownloadModelButton, enter = fadeIn(), exit = fadeOut()) {
    Column(
      ...
      DownloadAndTryButton(
        ...
        modelManagerViewModel = modelManagerViewModel,
        onClicked = {
          // After download completes, trigger model initialization
          Log.d("ModelDownloadStatusInfoPanel", "Download completed for model '${model.name}', triggering initialization")
        },
      )
    }
  }
