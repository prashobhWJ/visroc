...
constructor(
  ...
  fun addImportedLlmModel(info: ImportedModel) {
    ...
    for (task in
      ...
      if (
        ...
        Log.d(TAG, "Adding imported model '${model.name}' to task '${task.type.label}' (supports image: ${model.llmSupportImage})")
        task.models.add(model)
      } else {
        Log.d(TAG, "NOT adding imported model '${model.name}' to task '${task.type.label}' (supports image: ${model.llmSupportImage})")
      }
    ...
    viewModelScope.launch(Dispatchers.IO) {
      try {
        ...
        for (allowedModel in modelAllowlist.models) {
          ...
          // Skip adding online models to Ask Image task - only use imported models
          if (allowedModel.taskTypes.contains(TASK_LLM_ASK_IMAGE.type.id)) {
            // TASK_LLM_ASK_IMAGE.models.add(model) // Commented out - only use imported models
          }
  ...
  private fun createUiState(): ModelManagerUiState {
    ...
    val allImportedModels = dataStoreRepository.readImportedModels()
    val migratedModels = mutableListOf<ImportedModel>()
    var needsMigration = false
    
    for (importedModel in allImportedModels) {
      Log.d(TAG, "stored imported model: $importedModel")

      // Temporary migration: Enable image support for all imported models
      // This fixes the issue where previously imported models had supportImage = false by default
      val updatedImportedModel = if (!importedModel.llmConfig.supportImage) {
        Log.d(TAG, "Migrating model '${importedModel.fileName}' to support images")
        needsMigration = true
        importedModel.toBuilder()
          .setLlmConfig(
            importedModel.llmConfig.toBuilder()
      ...
      migratedModels.add(updatedImportedModel)

      // Create model.
      val model = createModelFromImportedModelInfo(info = updatedImportedModel)
      ...
      if (model.llmSupportImage) {
        ...
        Log.d(TAG, "Added imported model '${model.name}' to Ask Image task (supports image: true)")
      } else {
        Log.d(TAG, "NOT adding imported model '${model.name}' to Ask Image task (supports image: false)")
      }
    ...
    // Save migrated models if any were updated
    if (needsMigration) {
      Log.d(TAG, "Saving migrated models with image support enabled")
      dataStoreRepository.saveImportedModels(migratedModels)
    }
    ...
    Log.d(TAG, "TASK_LLM_ASK_IMAGE has ${TASK_LLM_ASK_IMAGE.models.size} models:")
    for (model in TASK_LLM_ASK_IMAGE.models) {
      Log.d(TAG, "  - ${model.name} (imported: ${model.imported}, supports image: ${model.llmSupportImage})")
    }
    return ModelManagerUiState(
      tasks = TASKS.toList(),
      modelDownloadStatus = modelDownloadStatus,
